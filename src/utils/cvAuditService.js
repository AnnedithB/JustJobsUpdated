...
export class CVAuditService {
  ...
  static validateFile(file) {
    ...
    // Check file type - only DOCX allowed
    const allowedTypes = [
      'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    ];
    
    if (!allowedTypes.includes(file.type)) {
      errors.push('Please upload a DOCX file only. PDF and DOC files are not supported.');
    }
  ...
  static getProcessingMethod(fileType) {
    ...
    return 'Text extraction with GPT-4o-mini analysis';
  }
} 